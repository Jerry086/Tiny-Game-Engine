...
void CollisionComponent::Update()
{
    ...
    // std::map<std::string, std::shared_ptr<CollisionComponent>>
    //     collision_components_list;
    // for (auto it = list.begin(); it != list.end(); it++)
    // {
    //     collision_components_list.emplace(
    //         it->first, std::dynamic_pointer_cast<CollisionComponent>(
    //                        it->second->m_components["3CollisionComponent"]));
    // }
    ...
    for (auto it = list.begin(); it != list.end(); it++)
    {
        ...
        if (other == nullptr || m_objectType == other->m_objectType)
        {
            continue;
        }
        else
        {
            ...
            {
                switch (GetType(other->m_objectType))
                {
                case wall:
                {
